...
const MostRecentCard = ({ ticket }: { ticket: ServingInfo | null }) => {
    ...
    return (
       <div className="bg-gold text-black p-4 rounded-lg text-center h-full flex flex-col justify-center">
            ...
            <div className="grid grid-cols-3 items-center text-center flex-grow">
                <div className="flex flex-col justify-center items-center h-full px-2">
                    <p className="text-9xl font-extrabold tracking-tight">{ticket.ticketNumber}</p>
        ...
        {tickets.length > 0 ? (
             <ScrollArea className="flex-grow">
                <ul className="divide-y divide-white/20">
                    {tickets.map((item) => (
                        <li key={`${item.ticketNumber}-${item.stationName}`} className="flex items-center p-4 text-6xl font-bold">
            ...
            {waitingTickets.length > 0 ? (
                <ul className="divide-y divide-white/20">
                    {waitingTickets.map((item, index) => (
                        <li key={item.id} className={cn("flex items-center p-4 text-6xl font-bold", index === 0 && "bg-white/10")}>
