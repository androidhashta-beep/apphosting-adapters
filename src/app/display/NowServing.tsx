       ...
       <div className="bg-gold text-black p-6 rounded-lg text-center h-full flex flex-col justify-center shadow-[0_0_30px_5px_hsl(var(--gold))]">
            ...
            <div className="grid grid-cols-3 items-center text-center flex-grow min-w-0 gap-4">
                ...
                <div className="flex flex-col justify-center items-center h-full px-2 min-w-0">
                    <p className="font-extrabold tracking-tight leading-none truncate text-8xl w-full">{ticket.ticketNumber}</p>
                    <p className="text-xl font-semibold mt-2">Ticket #</p>
                </div>
                {/* Service Column */}
                <div className="flex flex-col justify-center items-center h-full px-2 min-w-0">
                    <p className="text-4xl font-bold truncate w-full" title={ticket.serviceLabel}>{ticket.serviceLabel}</p>
                    <p className="text-xl font-semibold mt-2">Service</p>
                </div>
                {/* Window Column */}
                <div className="flex flex-col justify-center items-center h-full px-2 min-w-0">
                    <p className="text-4xl font-bold truncate w-full">{ticket.stationName}</p>
                    <p className="text-xl font-semibold mt-2">Window</p>
                </div>
            </div>
        </div>
    );
...
const AllServingList = ({ tickets }: { tickets: ServingInfo[] }) => (
    <div className="flex flex-col bg-black/30 rounded-lg overflow-hidden h-full">
        <h2 className="text-2xl font-bold text-center p-3 border-b-2 border-white/20 text-gold flex-shrink-0">
            Currently Serving
        </h2>
        <div className="grid grid-cols-3 px-6 py-2 font-semibold text-lg text-white/70 border-b border-white/20 flex-shrink-0">
        ...
        {tickets.length > 0 ? (
                ...
                <ul className="divide-y divide-white/10">
                    {tickets.map((item) => (
                        <li key={`${item.ticketNumber}-${item.stationName}`} className="grid grid-cols-3 items-center px-6 py-3 font-medium">
                            <span className="text-left truncate text-4xl font-bold">{item.ticketNumber}</span>
                            <span className="text-left truncate text-2xl">{item.serviceLabel}</span>
                            <span className="text-left truncate text-2xl">{item.stationName}</span>
                        </li>
                    ))}
                </ul>
            </ScrollArea>
            ...
            <div className="flex h-full items-center justify-center text-center text-slate-400 p-4">
                <p className="text-xl">No other tickets are being served.</p>
            </div>
        )}
    </div>
);
...
const WaitingQueue = ({ waitingTickets, serviceMap }: { waitingTickets: Ticket[], serviceMap: Map<string, string> }) => (
    <div className="flex flex-col bg-black/20 rounded-lg overflow-hidden h-full">
        <h2 className="text-2xl font-bold text-center p-3 border-b-2 border-white/20 flex-shrink-0">Waiting Queue</h2>
        <div className="grid grid-cols-2 px-6 py-2 font-semibold text-lg text-white/70 border-b border-white/20 flex-shrink-0">
            <span className="text-left">Ticket #</span>
            <span className="text-left">Service</span>
        </div>
        <ScrollArea className="flex-grow">
                ...
                <ul className="divide-y divide-white/10">
                    {waitingTickets.map((item, index) => (
                        <li key={item.id} className={cn(
                            ...
                            <span className="text-left truncate text-4xl font-bold">{item.ticketNumber}</span>
                            <span className="text-left truncate text-2xl">{serviceMap.get(item.type) || item.type}</span>
                        </li>
                    ))}
                </ul>
            ) : (
