...
const MostRecentCard = ({ ticket }: { ticket: ServingInfo | null }) => {
    ...
    return (
       <div className="bg-gold text-black p-4 rounded-lg text-center h-full flex flex-col justify-center">
            <p className="text-xl uppercase tracking-widest font-semibold mb-2 flex-shrink-0">Now Serving</p>
            <div className="grid grid-cols-3 items-center text-center flex-grow min-w-0">
                <div className="flex flex-col justify-center items-center h-full px-2 min-w-0">
                    <p className="text-7xl font-extrabold tracking-tight break-words">{ticket.ticketNumber}</p>
                    <p className="text-xl font-semibold mt-1">Ticket #</p>
                </div>
                <div className="flex flex-col justify-center items-center h-full px-2 min-w-0">
                    <p className="text-4xl font-bold break-words" title={ticket.serviceLabel}>{ticket.serviceLabel}</p>
                    <p className="text-xl font-semibold mt-1">Service</p>
                </div>
                <div className="flex flex-col justify-center items-center h-full px-2 min-w-0">
                    <p className="text-4xl font-bold break-words">{ticket.stationName}</p>
...
const AllServingList = ({ tickets }: { tickets: ServingInfo[] }) => (
    <div className="flex flex-col bg-black/30 rounded-lg overflow-hidden animate-pulse-slow border-2 border-gold shadow-lg h-full">
        <h2 className="text-xl font-bold text-center p-2 border-b-2 border-white/30 text-gold flex-shrink-0">
            Currently Serving
        </h2>
        <div className="grid grid-cols-3 px-4 py-2 font-bold text-base border-b border-white/20 flex-shrink-0">
        ...
        {tickets.length > 0 ? (
             <ScrollArea className="flex-grow">
                <ul className="divide-y divide-white/20">
                    {tickets.map((item) => (
                        <li key={`${item.ticketNumber}-${item.stationName}`} className="grid grid-cols-3 items-center p-3 font-bold text-3xl">
            ...
            <div className="flex h-full items-center justify-center text-center text-slate-300 p-4">
                <p className="text-lg">No tickets are being served right now.</p>
            </div>
        )}
    </div>
);
...
const WaitingQueue = ({ waitingTickets, serviceMap }: { waitingTickets: Ticket[], serviceMap: Map<string, string> }) => (
    <div className="flex flex-col bg-black/20 rounded-lg overflow-hidden h-full">
        ...
        <div className="grid grid-cols-2 px-4 py-2 font-bold text-base border-b border-white/20 flex-shrink-0">
            <span className="text-left">Ticket #</span>
            <span className="text-left">Service</span>
        </div>
        <ScrollArea className="flex-grow">
            {waitingTickets.length > 0 ? (
                <ul className="divide-y divide-white/20">
                    {waitingTickets.map((item, index) => (
                        <li key={item.id} className={cn("grid grid-cols-2 items-center p-3 font-bold text-3xl", index === 0 && "bg-white/10")}>
                            <span className="text-left break-words">{item.ticketNumber}</span>
